...
def train(
    ...
    for i in range(training_steps):
       ...
       # print(true_labels.unsqueeze(1))
        ...
        #print(true_labels)
        #print("L , %s" , generator_discriminator_out)
        generator_loss = loss(generator_discriminator_out, true_labels.unsqueeze(1).float())
        #print("k , %s", generator_loss)
        ...
        true_discriminator_loss = loss(true_discriminator_out, true_labels.unsqueeze(1).float())

        # add .detach() here think about this
        generator_discriminator_out = discriminator(generated_data.detach())
        generator_discriminator_loss = loss(
            generator_discriminator_out, torch.zeros(batch_size).unsqueeze(1).float()
        )
        discriminator_loss = (
